...
.window.window_bar_hidden .window_content{
  top: 0;
}`; var sdoc=document.createElement('style'); sdoc.innerText=css; document.body.appendChild(sdoc); /**
  ...
  this.reload = function(){
    ...
    bar.addEventListener("mousedown", function(e){
      ...
      if(WindowUtil.getProperty(options, "movable", true)){
        ...
        while(target != bar){
          if(target != target && target != toggle_close){
...
function advancedPrompt(){
  ...
  win = new Window("Advanced", {
			...
			win.content.id = "advanced_window";
			if (settings.flags){
				win.content.innerHTML=`
<p style="position: absolute; right: 45%; top: 20%; text-align: center;">Flags</p>
<input id="flags_input" style="position: absolute; right: 30%; top: 40%; text-align: center;" placeholder="--allow http_request" value="${settings.flags}">
`;
			}
			else {
				win.content.innerHTML=`
<p style="position: absolute; right: 45%; top: 20%; text-align: center;">Flags</p>
<input id="flags_input" style="position: absolute; right: 30%; top: 40%; text-align: center;" placeholder="--allow http_request">
`;
			}

      
      win.on('closed', function(){
if (document.querySelector('input').value.replace(/\s/g, '').length) {
        settings.flags = document.getElementById("flags_input").value;
}
      });
...
function interpretCode() {
  ...
  fetch('/api/run', {
    ...
    body: JSON.stringify({ code: code.value, flags: settings.flags})
  })
    .then(res => res.json())
    .then(data => {
      console.log(data)
